...
export function changeCandle(id, patch) { return commit(state.candles.map(c => c.id === id ? updateCandle(c, patch) : c), 'Candle updated.', id); }
/** @param {string} id */
export function purchase(id) { return commit(state.candles.map(c => c.id === id ? moveToShelf(c) : c), 'Candle moved to My Shelf.', id); }
/** @param {string} id */
...
export function loadDemo() {
  ...
  commit(examples.map(([name, brand, scentNotes, vesselStyle, burnStatus, rating, list]) => createCandle({ name, brand, scentNotes, vesselStyle, burnStatus, rating, list })), 'Six fictional demo candles added. Edit or delete them freely.');
}
