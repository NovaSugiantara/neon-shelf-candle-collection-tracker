...
function openForm(id) {
  ...
  if (candle) for (const key of ['name', 'brand', 'scentNotes', 'vesselStyle', 'burnStatus', 'rating']) field(key).value = String(candle[/** @type {keyof CandleInput} */ (key)] ?? '');
...
document.addEventListener('click', event => {
  ...
  switch (target.dataset.action) {
    ...
    case 'delete': { const candle = state.candles.find(c => c.id === id); if (!candle) break; deleting = candle.id; returnFocus = `delete-${candle.id}`; byId('delete-detail').textContent = `“${candle.name}” will be deleted from your collection. This cannot be undone.`; deletion.showModal(); break; }
